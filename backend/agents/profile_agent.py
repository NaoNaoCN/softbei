...
# 资源生成所需的最小必要字段（缺失则必须追问）
_REQUIRED_FOR_RESOURCE = {"kp_id"}
# 可用默认值兜底、不强制追问的字段
_DEFAULTS = {"cognitive_style": "text"}
...
async def run(state: AgentState) -> AgentState:
    ...
    # -- 2. 合并到数据库（stub：实际调用 profile_svc.merge_chat_updates）--
    # TODO: state.profile = await profile_svc.merge_chat_updates(
    #           uuid.UUID(state.user_id), updates, db)
    # 当前先将提取结果合并到 state.profile（内存级别）
