...
| # | 任务 | 状态 | 描述 | 文件 |
|---|---|---|---|---|
| P1-1 | **实现 Spark embedding** | ❌ 未完成 | 无 `_spark_embedding` 实现，embedding 仅支持 Qwen API 和本地 BGE-M3 | `backend/services/llm.py` |
| P1-2 | **本地 embedding 模型缓存** | ✅ 已完成 | `_get_embedding_model()` 单例模式，不再每次重新加载模型 | `backend/services/llm.py` |
| P1-3 | **聊天消息持久化** | ⚠️ 部分完成 | 写入了动态分表（`dynamic_chat`），未使用 `ChatMessage` 模型 | `backend/main.py:244` |
| P1-4 | **KG 节点数据导入** | ❌ 未完成 | 无批量导入接口，只有 `POST /kg/build` 从文档构建 | `backend/main.py:448` |
...
| P2-2 | **pathway.py kp_name 回退错误** | ✅ 已完成 | 已在 `_item_to_out()` 中正确取 `item.kp.name` | `backend/services/pathway.py:212` |
| P2-3 | **LearningRecord.action 写死为 "view"** | ❌ 未完成 | `record_learning` 硬编码 `"action": "view"`，忽略传入值 | `backend/services/resource.py:183` |
| P2-4 | **多选题答案批改逻辑** | ❌ 未完成 | 多选题答案列表转字符串后与数据库不匹配 | `backend/main.py:661` |
| P2-5 | **前端静默失败** | ⚠️ 部分修复 | `col_refresh` 已定义；统计 API 已修复；`cognitive_style` 索引已修复 | `streamlit_app/pages/*.py` |
...
| P3-3 | **单元测试覆盖率** | ⚠️ 部分完成 | agents/、db/、schemas、crud、rag、services/ 已有测试；llm.py、main.py 仍缺失 | `tests/` |
...
| `backend/services/` | `test_services.py` | ~35 |
...
`backend/services/resource.py:183` 硬编码 `"action": "view"`，应改为：
```python
